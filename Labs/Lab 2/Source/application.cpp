...
void App::drawShapes() {
	cout << "Number of shapes: " << shapes.size() << '\n';
	glFlush();
...
Shape* App::createShape(vector < int > input) {
	...
	switch (input[0]) {
	case 0:
		result = new LineDDA(input[1], input[2], input[3], input[4]);
		cout << "Create LineDDA " << input[1] << ' ' << input[2] << ' ' << input[3] << ' ' << input[4] << '\n';
		break;
	case 1:
		result = new LineBresenham(input[1], input[2], input[3], input[4]);
		cout << "Create LineBresenham " << input[1] << ' ' << input[2] << ' ' << input[3] << ' ' << input[4] << '\n';
		break;
	case 2:
		result = new CircleMidpoint(input[1], input[2], input[3]);
		cout << "Create CircleMidpoint " << input[1] << ' ' << input[2] << ' ' << input[3] << '\n';
		break;
	case 3:
		result = new ElipseMidpoint(input[1], input[2], input[3], input[4]);
		cout << "Create ElipseMidpoint " << input[1] << ' ' << input[2] << ' ' << input[3] << ' ' << input[4] << '\n';
		break;
	case 4:
		result = new ParabolaMidpoint(input[1], input[2], input[3]);
		cout << "Create ParabolaMidpoint " << input[1] << ' ' << input[2] << ' ' << input[3] << '\n';
		break;
	case 5:
		result = new HyperbolaMidpoint(input[1], input[2], input[3], input[4]);
		cout << "Create LineDDA " << input[1] << ' ' << input[2] << ' ' << input[3] << ' ' << input[4] << '\n';
		break;
	default:
		cout << "WTF\n";
